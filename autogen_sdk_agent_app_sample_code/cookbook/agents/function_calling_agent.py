...
REQUIRED_ENV_VARIABLES = ["DATABRICKS_HOST", "DATABRICKS_TOKEN"]
...
class FunctionCallingAgent(mlflow.pyfunc.PythonModel):
    ...
    def __init__(
        ...
        if not all([os.environ.get(env) for env in REQUIRED_ENV_VARIABLES]):
            logging.error(f"Environment variables not set. Please set the following environment variables: {REQUIRED_ENV_VARIABLES}")
            raise Exception()
    ...
    def create_agents(self, chat_history):
        ...
        api_base_url = f"{os.environ['DATABRICKS_HOST']}/serving-endpoints/"

        config_list = [{"model": llm_config.llm_endpoint_name, 
        ...
        assistant = ConversableAgent(
            ...
            llm_config={"config_list": config_list, **llm_config.llm_parameters.dict()},
